...
def create_kind_table(cursor, db):
    print("### KIND VERSION 2026-01-DEBUG ###")

    try:
        print("[DEBUG] create_kind_table: START")

        print("[DEBUG] Disabling FOREIGN_KEY_CHECKS")
        cursor.execute("SET FOREIGN_KEY_CHECKS = 0")

        print("[DEBUG] Dropping table kind if exists")
        cursor.execute("DROP TABLE IF EXISTS kind")

        print("[DEBUG] Creating table kind")
        cursor.execute("""
            CREATE TABLE kind (
        ...
        db.commit()
        print("[OK] Ο πίνακας kind δημιουργήθηκε επιτυχώς")

    except Exception as e:
        print("[ERROR] create_kind_table FAILED")
        print("[MYSQL ERROR]", e)
